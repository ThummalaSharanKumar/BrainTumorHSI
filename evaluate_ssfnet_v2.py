...
class HSIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # NOTE: Augmentation should NOT be applied during evaluation
            ...
            # Ensure reducer input is correct dtype if needed (only NMF requires float64)
            pixel_data = pixels
            if isinstance(self.reducer, joblib.load('nmf_model.pkl').__class__): # Rough check for NMF type
            ...
            n_components = getattr(self.reducer, 'n_components_', 15) # Get components safely
...
# --- SSF-NET V2 EVALUATION CONFIGURATION ---
EVAL_CONFIG = {
    "BATCH_SIZE": 2,
    ...
    "MODEL_PATH": 'best_ssfnet_model_v2.pth', # Path to the newly trained model
}

# --- MAIN EXECUTION SCRIPT ---
if __name__ == "__main__":
    print(f"--- Starting Final Evaluation of SSF-Net Model (v2 with LR Scheduler) ---")
    ...
    if not os.path.exists(EVAL_CONFIG['MODEL_PATH']):
        ...
        print("!!! Please ensure the SSF-Net (v2) training completed successfully.")
        sys.exit()
        
    reducer = joblib.load(EVAL_CONFIG["REDUCER_PATH"])
    test_dataset = HSIDataset(csv_file='test.csv', reducer=reducer)
    # Use num_workers=0 for stability on local machine
    ...
    with torch.no_grad():
        for hsi_spatial, hsi_spectral, mask in tqdm(test_loader, desc="Evaluating SSF-Net v2 on Test Set"):
    ...
    print("\n--- SSF-Net v2 Evaluation Complete ---")
