...
# Ensure this points to the ssf_net.py file that INCLUDES the SEBlock
...
class HSIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # NMF requires float64
            pixel_data = pixels
            # A more robust check for NMF instance
...
# --- SSF-NET V3 (ATTN) EVALUATION CONFIGURATION ---
EVAL_CONFIG = {
    "BATCH_SIZE": 2,
    ...
    # --- Path to the attention model ---
    "MODEL_PATH": 'best_ssfnet_model_v3_attn.pth',
    # ------------------------------------
}

# --- MAIN EXECUTION SCRIPT ---
if __name__ == "__main__":
    print(f"--- Starting Final Evaluation of SSF-Net Model (v3 with Attention) ---")
    ...
    if not os.path.exists(EVAL_CONFIG['MODEL_PATH']):
        ...
        print("!!! Please ensure the SSF-Net (v3) training completed successfully.")
        sys.exit()
    ...
    try:
        ...
        print("!!! This might happen if the saved model architecture doesn't match the current SSFNet class (check SEBlock).")
        sys.exit()
    ...
    with torch.no_grad():
        for hsi_spatial, hsi_spectral, mask in tqdm(test_loader, desc="Evaluating SSF-Net v3 (Attn) on Test Set"):
    ...
    print("\n--- SSF-Net v3 (Attention) Evaluation Complete ---")
